...
void output_blank_file(void); // 10 * 10 matrix
...
void output() {
    vector<vector<string>> out_matrix(10, vector<string> (10));
...
void output_blank_file() { // 10 * 10 matrix
...
vector<string> split_by_dot(const string &str) {
    ...
    ret.emplace_back(tmp);
    assert(ret.size() == 10); // assure 10 column each row
    return ret;
}
...
vector<vector<string>> read_from_txt() {
    ...
    in.close();
    in.clear();
    assert(ret.size() == 10); // assure that 10 row
    return ret;
}
...
void output_matrix(const vector<vector<uint32_t>> &matrix) {
    for (uint32_t i = 0; i < 10; i++) {
        for (uint32_t j = 0; j < 10; j++)
            cout << setw(2) << ((matrix[i][j] == 0) ? "" : to_string(matrix[i][j])) << " ";
        cout << endl;
    }
}
...
void get_candidate_values_and_constraints(const vector<vector<string>> &matrix,
                                          ...
                                          vector<Constraint> &constraints_vct) {
    uint32_t cur_id = 0;
    candidates_map.resize(10, vector<uint32_t> (10, 0));
    for (uint32_t i = 0; i < 10; i++) {
        for (uint32_t j = 0; j < 10; j++) {
            auto &e = matrix[i][j];
            if (e.empty()) continue;
    ...
    for (int32_t i = 0; i < 10; i++) {
        for (int32_t j = 0; j < 10; j++) {
            ...
            int x = i + dx;
            int y = j + dy;
            while (0 <= x && x < 10 && 0 <= y && y < 10 && candidates_map[x][y] != 0) {
